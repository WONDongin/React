...
const BoardList = () => {
    const [bList, setBList] = useState([]); // 게시글 목록
    const [boardCount, setBoardCount] = useState([]); // 게시글 등록건수
    const [start, setStart] = useState([]); // 페이지 시작 번호
    const [end, setEnd] = useState([]);     // 페이지 종료 번호
    ...
    */
    ...
    /* 
    useEffect
    - 특정값이 변경될때만 실행하도록 설정 가능함
    - 빈배열([])인 경우 처음 화면에 나타날때만 한번 실행함
    */
    useEffect(()=> {
    ...
    const getBoardList = () => {
        if (queryString.length === 0) {
            queryString = "?boardid=" + boardid; // 서버 전송시 파라미터 값 설정
        }
        // url : BackEnd 서버 주소
        ...
        .then((json) => {
            ...
            setBottomLine(json.bottomLine) // 화면에 보여질 페이지의 갯수
            setMaxPage(json.maxPage) // 최대 페이지
            setBoardName(json.boardName) // 게시판 이름
        })
    }
    // start 부터 end까지 값을 배열로 리턴.현재 화면의 페이지 번호 목록
        ...
        <div>
            <div className="container">
                ...
                <p className="text-right">
                    <a className="btn btn-primary" href={`/board/boardForm/${boardid}`}>게시판 입력</a>
                </p>
                <table className="table table-bordered">
                    ...
                    */}
                    {Array.isArray(bList) && bList.length > 0 ? ( // 게시글이 있는 경우
                        // b : board 객체, index : 순서
                        bList.map((b, index) => (
                        <tr key={index}>
                        ...
                        <tr>
                            <td colSpan="6" className="text-center">게시물이 없습니다.</td>
                        </tr>
                    )}
                    </tbody>
                </table>
                ...
                <ul className="pagination center" style={{ justifyContent: "center" }}>
                    <li className={start <= bottomLine ? "page-item disabled" : "page-item"}>
                        <a className="page-link" href={"/board/boardList?page=" + (start - bottomLine)}>Previous</a>
                    </li>
                    {/* getPage(start, end) : start, end 까지의 배열*/}
                    {getPage(start, end).map((p) => (
                        <li key={p} className={pageInt === p ? "page-item active" : "page-item"}>
                        <a className="page-link" href={"/board/boardList?page=" + p}>{p}</a>
                        </li>
                    ))}

                    <li className={end >= maxPage ? "page-item disabled" : "page-item"}>
                        <a className="page-link" href={"/board/boardList?page=" + (start + bottomLine)}>Next</a>
                    </li>
                </ul>
            </div>
        </div>
